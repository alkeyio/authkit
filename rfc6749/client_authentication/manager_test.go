...
func TestManager_Authenticate(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		c, err := m.Authenticate(context.Background(), r, map[types.ClientAuthMethod]bool{types.ClientBasicAuthentication: true}, "token")
	...
	t.Run("error_when_method_is_not_supported", func(t *testing.T) {
		c, err := m.Authenticate(context.Background(), r, map[types.ClientAuthMethod]bool{"password": true}, "token")
		assert.Nil(t, c)

		var authErr *autherrors.AuthKitError
